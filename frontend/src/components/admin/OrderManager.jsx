  ...
  return (
    <div className="space-y-6">
      ...
      {readyOrders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 stagger relative">
          {readyOrders.map((order, i) => {
            const isExpanded = expandedOrderId === order.id
            const isAnyExpanded = expandedOrderId !== null
            const itemCount = order.order_items?.reduce((acc, curr) => acc + curr.quantity, 0) || 0

            return (
              <div 
                key={order.id} 
                onClick={() => setExpandedOrderId(isExpanded ? null : order.id)}
                className={`card-lifted overflow-hidden flex flex-col bg-white/80 backdrop-blur-xl border-slate-200/60 transition-all duration-300 origin-top cursor-pointer
                  ${isAnyExpanded && !isExpanded ? 'opacity-40 grayscale-[0.5] scale-[0.97]' : 'animate-slide-up shadow-sm hover:shadow-md'}
                  ${!isExpanded ? 'h-32' : 'h-auto shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] scale-[1.02] z-10'}
                `} 
                style={!isAnyExpanded ? { animationDelay: (i * 50) + 'ms' } : {}}
              >
                
                {/* Header (Always Visible) */}
                <div className={`p-5 flex justify-between items-start transition-colors ${isExpanded ? 'bg-slate-50/80 border-b border-slate-100' : 'bg-white h-full'}`}>
                  <div className="flex flex-col justify-between h-full">
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md shadow-sm ${isExpanded ? 'bg-orange-600 text-white' : 'bg-slate-900 text-white'}`}>
                      ...
                      <h3 className="font-black text-lg text-slate-800 leading-tight capitalize truncate max-w-[150px]">{order.customer_name}</h3>
                    </div>
                    {!isExpanded && (
                      <div className="mt-auto pt-2 flex items-center gap-2 text-xs font-bold text-slate-500">
                        <i className="fa-solid fa-cookie-bite"></i> {itemCount} items
                      </div>
                    )}
                  </div>
                  <div className="text-right flex flex-col items-end justify-start h-full">
                    ...
                    <p className="text-slate-400 text-[10px] uppercase font-bold tracking-widest mb-auto">Total</p>
                    
                    {!isExpanded && (
                      <span className="mt-auto w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 border border-slate-100 shadow-inner group-hover:bg-slate-100 transition-colors">
                ...
                <div 
                  className={`flex-grow bg-white overflow-hidden transition-all duration-300 ease-in-out`}
                  style={{ maxHeight: isExpanded ? '1000px' : '0px', opacity: isExpanded ? 1 : 0 }}
                  onClick={e => e.stopPropagation()} // Prevent collapse when clicking inside the form
                >
                  <div className="p-5 border-b border-slate-100">
                    ...
                    {order.special_instructions && (
                      <div className="bg-orange-50 p-3 border border-orange-100 rounded-xl flex gap-2 items-start mt-4 shadow-inner">
                  ...
                  <div className="p-5 bg-slate-50 space-y-4">
                      ...
                      <div className="flex bg-slate-200/50 p-1 rounded-xl">
                        {['cash', 'upi', 'card'].map(m => {
                          ...
                          return (
                            <button
                              key={m}
                              onClick={(e) => { e.stopPropagation(); setPaymentMethod({ ...paymentMethod, [order.id]: m }) }}
                              className={`flex-1 py-2 px-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${
                                isSelected
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={(e) => { e.stopPropagation(); processPayment(order) }}
                        disabled={processing === order.id}
                        className="flex-[2] btn-success !py-3 !text-sm flex items-center justify-center gap-2"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 hover:text-slate-900 transition-colors font-black text-sm active:scale-95 flex items-center justify-center shadow-sm"
                        title="Print Bill"
                      >
